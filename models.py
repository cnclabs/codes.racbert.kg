...
class Decoder(torch.nn.Module):
    ...
    def forward(self, ins_emb, rel_emb, sample, metric=None):
        ...
        if type(ins_emb) == list:
            return func([ins_emb[i] for i in sample[:, 0].tolist()], [ins_emb[i] for i in sample[:, 1].tolist()], metric)
        else:
            return func(ins_emb[sample[:, 0]], ins_emb[sample[:, 1]], metric)
...
class Align(torch.nn.Module):
    ...
    def forward(self, e1, e2, metric=None):
        if not metric:
            pred = - torch.norm(e1 - e2, p=self.p, dim=1)
        elif metric == "cos":
            pred = F.cosine_similarity(e1, e2)
        elif metric == "seq":
            assert type(e1[0]) == str
            pred = SequenceMatcher(None, e1, e2).ratio()
        return pred
