...
for name, param in list(sbert.named_parameters())[:-2]:
    # print(name)
...
class Experiment:
    ...
    def train_and_eval(self):
        ...
        if self.args.two_views == 1:
            ...
            if self.args.contras_flag == 1:
                params1 = nn.ParameterList((list(self.fc1.parameters())) + (list(self.fc2.parameters())) + + (list(sbert.parameters())))
    ...
    def train_and_eval_val(self,sb_fine_tune):
        if sb_fine_tune==1:
            self.init_emb_sb()
        else:
            self.init_emb()
...
def cmab(lefts, score_dicts, train_mapping, trained, ouf):
    ...
    _, H1 = experiment.train_and_eval_val(sb_fine_tune=0)
    H1_pre = H1*100
    ...
    experiment = Experiment(args=args)
    experiment.vali = True
    _, H1 = experiment.train_and_eval_val(sb_fine_tune=1)
    H1 = H1 * 100
    gap = H1 - H1_pre
    if gap < 0:
        gap = 0.0
